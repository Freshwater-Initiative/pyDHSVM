...
def plot_results(results, threshold=0.2):
    ...
    with open('sensitivity_data_run_raw_liv.txt', 'w') as f:
    ...
    fig.savefig('FAST_sensitivity_run_raw_liv.png',dpi=300)
...
#Files and their input paths. If there are errors, please CHECK PATH
...
# Cheap logging
### OPTIMIZATION PARAMETERS -- IF NEEDED, CHANGE THEM HERE! PARAMETER NAMES WILL BE REFLECTED IN THE SQLITE DB###
def log(s):
...
class fast_run_setup(object):
    def __init__(self, parallel='seq'):
        ...
        self.params = [spotpy.parameter.Uniform('Rain_threshold',low=-3 , high=-1,  optguess=-2),
                      ...
                      # spotpy.parameter.Uniform('Lateral_Conductivity_62',low=0.00001, high=0.01,  optguess=0.00017),
                       ...
                       #spotpy.parameter.Uniform('Precipitation_Multiplier',low=0.000005, high=0.00003,  optguess=0.0000,                       spotpy.parameter.Uniform('Maximum_Resistance_9',low=500, high=3000,  optguess=3000),
    ...
    def simulation(self, x):
        ...
        #copy_tree(".", child_dir)
        shutil.copytree(".", child_dir, ignore=shutil.ignore_patterns(DIR_PREFIX + "*", "fast_dhsvm_parallel_run*"))
        ...
        #write DREAM parameter input to config file. These values are from SPOTPY, see above. Check config file has these exact parameters. 
...
sampler=spotpy.algorithms.fast(fast_run, dbname='fast_dhsvm_parallel_run_raw_liv', dbformat='sql', parallel='mpc')
sampler.sample(10) # Decrease sample size to run the script faster. Lower number will not render any errors but there will not be sufficient plots on the notebooks

results = sampler.getdata()
results_data = pd.DataFrame(results)
results_data.to_csv('fast_sensitive_params_run_raw_liv.csv')
